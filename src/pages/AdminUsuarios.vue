<template>
  <q-page class="column dark full-heigth">
    <div class="text-h4 text-center text-bold q-pa-sm q-mt-md">Registrar Empleado</div>
    <div class="column q-pa-md">
      ...
      <div class="text-h4 text-center text-bold q-pa-sm q-mt-md">Lista de Empleados</div>
      <q-table
      ...
      >
        <template v-slot:top>
          <q-btn color="primary" dense flat :disable="loadingEmpleados || !(selected.length == 1)" icon="edit"/>
          <q-btn color="red-5" dense flat :disable="loadingEmpleados || !(selected.length > 0)" icon="delete"/>
          <q-space />
...
export default {
  ...
  computed: {
    // ...mapState('users', ['empleados', 'loadingempleados']),
    pagesNumber () {
      return Math.ceil(this.empleados.length / this.pagination.rowsPerPage)
    },
    disableButton () {
      if (this.step === 1) {
